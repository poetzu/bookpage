...
<script type="text/javascript">
 	...
 	function checkForm(){
		...
		var getPass = RegExp(/^[a-zA-Z0-9]{4,12}$/);
			...
			// 비밀번호 공백 확인
 			...
 			//비밀번호 유효성 
 			if (!getPass.test($("#passwd").val())) {
 				alert("4-12자 영문자+숫자의 비밀번호를 설정해주세요");
 				$("#passwd").val("");
 				$("#passwd").focus();
 				return false;
 			}
...
<!-- 본문내용시작 -->
	<form action="joinPro.jsp" method="post" id="join" onsubmit="return checkForm();" name="fr">
		 <label> <sup>*</sup> 는 필수 입력 바랍니다. <br><br></label>
		 <hr>
		 <span><br><sup>*</sup> 이름 </span><br><br>
		 <input type="text" id="id" name="name" class="name" placeholder="NAME" required autofocus><br>
		<span><br><sup>*</sup> 아이디 </span><br><br>
		 <input type="text" id="name" name="id" class="id" placeholder="ID" required>
		 <input type="button" id="btn" value="아이디중복확인" class="dup" onclick="winopen()"><br>
		<span><br><sup>*</sup> 비밀번호 (4-12자 영문자+숫자) </span><br><br>
		 <input type="password" id="passwd" name="passwd" class="passwd" placeholder="PASSWORD" required><br>
		 <input type="password" id="passwd2" name="passwd2" class="passwd2"placeholder="PASSWORD CHECK" required><br>
		<span><br><sup>*</sup> 출생년도 (4자리)</span><br><br>
		 <input type="text" id="age" name="age" class="age" placeholder="출생년도" required><br><br>
		<span><sup>*</sup>성별</span><br><br>
		 <label for="male">
			<div id="GGender">
		...
		<span><sup>*</sup> 휴대폰번호 </span><br><br>
		<input type="text" id="phone" name="phone" class="phone" placeholder="PHONE NUMBER" required><br><br>
		<span> 이메일 </span><br><br>
		<input type="text" id="email" name="email" class="email" placeholder="E-MAIL" required><br><br><br>
		<span>주소</span><br><br>
		<div id="post">
				<input type="text" name="postcode" id="postcode" placeholder="우편번호" class="textWithBtn">
